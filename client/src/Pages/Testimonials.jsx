...
const StatCard = ({ icon: Icon, value, label, delay }) => (
  <motion.div
    ...
    whileTap={{ scale: 0.95 }}
    layout
    className="text-center p-6 rounded-lg bg-gray-50 dark:bg-zinc-900 group hover:bg-pink-50 dark:hover:bg-pink-900/20 transition-all border border-transparent dark:border-zinc-800 shadow-md"
  >
    <motion.div
...
const Testimonials = () => {
  ...
  const reviewHighlights = [
    ...
    { title: "Great Value", count: 1542, color: "text-pink-500 dark:text-pink-400", bgColor: "bg-pink-50 dark:bg-pink-900/20" },
  ];
      ...
      <section className="pt-16 pb-4">
        <div className="container mx-auto px-4">
          <motion.div variants={fadeIn} initial="initial" whileInView="whileInView" className="text-center max-w-3xl mx-auto">
            <motion.div whileHover={{ scale: 1.05, rotate: [0, 1, -1, 0] }} className="inline-flex items-center gap-2 px-4 py-2 bg-pink-100 dark:bg-pink-900/30 rounded-full mb-6 cursor-pointer hover:bg-pink-200 dark:hover:bg-pink-800 transition-colors duration-200">
              <Quote className="w-5 h-5 text-pink-500" />
              <span className="text-pink-700 dark:text-pink-400 font-medium">Customer Stories</span>
            </motion.div>
        ...
        <div className="container mx-auto px-4">
          <div className="relative">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={`rounded-2xl p-8 md:p-12 shadow-lg border transition-colors ${
                  activeIndex % 2 === 0 ? "bg-white dark:bg-zinc-900" : "bg-pink-50 dark:bg-pink-900/20"
                }`}
              >
            ...
            <div className="flex justify-center gap-4 mt-6">
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={prevTestimonial} className="p-3 rounded-full bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-pink-50 dark:hover:bg-pink-900/30 hover:border-pink-200 dark:hover:border-pink-900/50 transition-all shadow-sm">
                <ChevronLeft className="w-6 h-6" />
              </motion.button>
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={nextTestimonial} className="p-3 rounded-full bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-pink-50 dark:hover:bg-pink-900/30 hover:border-pink-200 dark:hover:border-pink-900/50 transition-all shadow-sm">
            ...
            <div className="flex justify-center mt-4 gap-2">
              {testimonials.map((_, idx) => (
                <motion.span
                  key={idx}
                  className={`w-3 h-3 rounded-full ${activeIndex === idx ? "bg-pink-500 dark:bg-pink-400 animate-pulse" : "bg-gray-300 dark:bg-zinc-700"}`}
                  layout
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <motion.div 
            ...
            className="bg-pink-500 rounded-2xl p-8 md:p-12 text-center text-white shadow-xl shadow-pink-500/20 hover:shadow-2xl transition-all"
          >
            <h2 className="text-3xl font-bold mb-4">Ready to Experience It Yourself?</h2>
            <p className="max-w-2xl mx-auto mb-8 text-pink-100 font-medium">
            ...
            <Link to="/models">
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }} 
                className="bg-white dark:bg-zinc-100 text-pink-500 px-8 py-3 rounded-lg font-bold hover:bg-pink-50 dark:hover:bg-white transition-all shadow-lg"
              >
                Book Now
              </motion.button>
            </Link>
